...
function AmeJob(jobProfile, outputLocation, jobInput, asyncEndpoint) {
    this["@context"] = internalContext;
    this.type = "AmeJob";
...
function TransformJob(jobProfile, outputLocation, jobInput, asyncEndpoint) {
    this["@context"] = internalContext;
...
function getServices(callback) {
    ...
    httpGet(serviceRegistryServicesURL, callback);
}
            ...
            services.forEach(service => {
                ...
                hasResources.forEach(hasResource => {
                    if (hasResource.resourceType === type) {
                        resourceURLs.push(hasResource.locator);
                    }
                });
            });
